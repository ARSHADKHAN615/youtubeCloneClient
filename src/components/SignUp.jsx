...
const SignUp = () => {
  ...
  const [userFindLoading, setUserFindLoading] = useState(false);
  ...
  const usernameExist = async (username) => {
    try {
      setUserFindLoading(true);
      const res = await api.get(`auth/usernameExist/${username}`);
      if (res.data) {
        setUsernameText(res.data);
      }
      setUserFindLoading(false);
    } catch (error) {
      console.log(error);
      setUsernameText({ message: error.response.data.message });
      setUserFindLoading(false);
    }
  };
  
  useEffect(() => {
    usernameExist(formData.name);
  }, [formData.name]);
  ...
  if (formData.name == "") {
    availability = (
      <p className="text-gray-500 text-xs mt-1">Enter username to check</p>
    );
  } else if (userFindLoading) {
    availability = (
    ...
    if (usernameText.message == "Username Not available") {
      availability = (
